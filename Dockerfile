...
RUN wget --no-check-certificate -O /tmp/google-chrome-stable.deb https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb && \
    ...
    groupadd -r _crd_network && \
    useradd -r -g _crd_network _crd_network || true && \
    apt-get install -y --no-install-recommends /tmp/google-chrome-stable.deb || true && \
    dpkg --unpack /tmp/chrome-remote-desktop.deb && \
    sed -i 's/adduser --system "$CRD_NETWORK_USER"/useradd -r "$CRD_NETWORK_USER" || true/' /var/lib/dpkg/info/chrome-remote-desktop.postinst || true && \
    apt-get install -f -y && \
